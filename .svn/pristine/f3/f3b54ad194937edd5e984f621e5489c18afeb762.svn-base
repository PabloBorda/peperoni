...

{
  ...
  :login => Proc.new {    
    puts "|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"
    client = Mongo::MongoClient.new('localhost', 27017).db('pborda', :strict => false)['mts-tickets-indexed-by-paragraph']
    #puts "LOGGED IN !!"                                                             
    #client['pborda']
    
  },
  :retriever => Proc.new { 
      |session_object,params|
      coll = session_object
      ...
      puts "SESSION OBJECT FROM RETRIEVER " + session_store.to_json
  ...
  :helper_methods_exposed_to_url => [{:find_ticket_feed => Proc.new {
                                                             ...
                                                             puts "SESSION STORE FROM helper_methods_exposed_to_url " + session_store.to_json
                                                             coll = session_store[session[:username] + "|||" + "mongo_mts_tickets_feed"][:session]                                                             
                      ...
                      {:record_selection_feed => Proc.new {                      
                         ...
                         tik = params[:ticket_number]                        
                         client = Mongo::MongoClient.new('localhost', 27017).db('pborda', :strict => false)
                         coll = client['mts-tickets-indexed-by-paragraph']
